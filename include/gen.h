...
typedef struct {
    char *buf; // buffer
    size_t n;  // number of chars in the buffer
} Text;

// Same as printf but the chars are written in the text buffer not on the screen
void Text_write(Text *text, const char *fmt, ...);

// Deletes the chars from a buffer
void Text_clear(Text *text);

extern Text tBegin // for header file and variables
...
, tFnHeader // used temporary at the function header generation
;

// these pointer will point to different buffers
